...
export default function HelpModal({ appMode, onClose }: HelpModalProps) {
  ...
  const handleCopyWechat = async () => {
    try {
      await copyTextToClipboard('RoyalKnightKilito')
      showToast('微信号已复制', 'success')
    } catch (err) {
      showToast(getClipboardFailureMessage('复制微信号失败', err), 'error')
    }
  }
      ...
      >
        ...
        <div className="pt-4 border-t border-gray-200 dark:border-white/[0.08] flex justify-center">
          <button
            ...
            className="flex items-center gap-2 rounded-lg px-2 py-1 text-sm font-medium text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-white/[0.06] dark:hover:text-gray-100 group"
            title="点击复制微信号"
          >
